...
@section('title', 'Agregar Ficha de Caracterización')
...
<div class="card card-primary">
    ...
    <form id="formFichaCreate" action="{{ route('fichas.store') }}" method="POST" novalidate>
        @csrf
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control" id="codigo" name="Codigo"
                    value="{{ old('Codigo') }}" required aria-required="true">
            </div>

            <div class="form-group">
                <label for="denominacion">Denominación</label>
                <input type="text" class="form-control" id="denominacion" name="Denominacion"
                    value="{{ old('Denominacion') }}" maxlength="100" required aria-required="true">
            </div>

            <div class="form-group">
                <label for="cupo">Cupo</label>
                <input type="number" class="form-control" id="cupo" name="Cupo"
                    value="{{ old('Cupo') }}" required aria-required="true">
            </div>

            <div class="form-group">
                <label for="fechainicio">Fecha Inicio</label>
                <input type="date" class="form-control" id="fechainicio" name="Fechainicio"
                    value="{{ old('Fechainicio') }}" required aria-required="true">
            </div>

            <div class="form-group">
                <label for="fechafin">Fecha Fin</label>
                <input type="date" class="form-control" id="fechafin" name="Fechafin"
                    value="{{ old('Fechafin') }}" required aria-required="true">
            </div>

            <div class="form-group">
                <label for="observaciones">Observaciones</label>
                <textarea class="form-control" id="observaciones" name="Observaciones" rows="3" maxlength="200">{{ old('Observaciones') }}</textarea>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const form = document.getElementById('formFichaCreate');
                ...
                .then(data => {
                    if (data && data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: '¡Creada!',
                            text: data.message || 'La ficha se creó correctamente.',
                            showConfirmButton: false,
                            timer: 1600
                        }).then(() => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: data && data.message ? data.message : 'No se pudo crear la ficha.'
                        });
                    }
                })
                .catch(error => {
